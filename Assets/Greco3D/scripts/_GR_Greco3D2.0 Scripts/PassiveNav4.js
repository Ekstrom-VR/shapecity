...
function RotatePlayer()
{
     ...
     ChangeAccelR(0,1,1);
     if(hitDist < 45){
     timer = Random.Range(20,30) / 10.0;
     }
     else 
     {
      timer = Random.Range(5,20) / 10.0;

     }
   	 StartCoroutine("Timer");
     action = "Rotate"; 
     
     print("DB...new action: rotate; timer: "+timer);
...
var targetFound : boolean = false;
		if(Physics.Raycast (transform.position,transform.TransformDirection(Vector3.forward) ,hit, 500)){
				...
				if(rayTag == "store" || rayTag == "boundary"){ 
         		hitDist = hit.distance;
         		randPerc = Random.Range(30,60) / 100.0;
         		timer =  (hitDist/tranVel) *randPerc; 
         		print("FWD Perc: " + randPerc + "Timer: " + timer + ',hitDist: ' + hitDist + 'tranVel: ' + tranVel + ',store: ' + rayStore);
         		}
...
var hit : RaycastHit;
   ...
   if(!objectHit){
		...
		if (Physics.Raycast (transform.position, bwd,hit, 5)) {
			if(action != 'Rotate'){
...
//		storeListc[i] = Control.curStoreList[2];
		...
		//For rotation
		rotVelList[i] = Random.Range(50,51);//Rotation velocity
