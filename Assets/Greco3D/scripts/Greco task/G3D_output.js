...
//Initialize variables
	...
	var newFile = System.IO.File.Create( "Data/CitiEnc/" +subj + "_" + G3D_task.version + "_output.txt");
	newFile.Close();
	}
}
...
function Update () {
     ...
     //Get city type
     trial_order = G3D_task.trial_list[G3D_task.currRun];
     ...
     else if(G3D_task.cnt < G3D_task.numTrials)  {
      
	     priorCity = trial_order[G3D_task.cnt-1];
     ...
     currCity = trial_order[G3D_task.cnt];
     }
     

	if (Input.GetKeyDown('left'))
	{
    print ('left key down at ' + G3D_cityMorph.stopwatch);
	resp_key = 's';
	...
	{
	print ('right key down at ' + G3D_cityMorph.stopwatch);
	resp_key = 'd';
	resp_time = G3D_cityMorph.stopwatch;
	resp_key_list[cntR] = resp_key;
	resp_time_list[cntR] = G3D_cityMorph.stopwatch;
	...
	{
	print ('V key down at ' + G3D_cityMorph.stopwatch);
	resp_key = '1';
	resp_time = G3D_cityMorph.stopwatch;
	resp_key_list[cntR] = resp_key;
	resp_time_list[cntR] = G3D_cityMorph.stopwatch;
	...
	{
	print ('B key down at ' + G3D_cityMorph.stopwatch);
	resp_key = '2';
	resp_time = G3D_cityMorph.stopwatch;
	resp_key_list[cntR] = resp_key;
	resp_time_list[cntR] = G3D_cityMorph.stopwatch;
	...
	{
	print ('N key down at ' + G3D_cityMorph.stopwatch);
	resp_key = '3';
	resp_time = G3D_cityMorph.stopwatch;
	resp_key_list[cntR] = resp_key;
	resp_time_list[cntR] = G3D_cityMorph.stopwatch;
	...
	{
	print ('M key down at ' + G3D_cityMorph.stopwatch);
	resp_key = '4';
	resp_time = G3D_cityMorph.stopwatch;
	resp_key_list[cntR] = resp_key;
	resp_time_list[cntR] = G3D_cityMorph.stopwatch;
	...
	acc_curr = 0;
 	...
 	line =  cntT+ "\t " + G3D_task.cnt + "\t "+ G3D_task.currRun +"\t " + G3D_cityMorph.startTime + "\t" + resp_key + "\t" + resp_time + "\t" + trial_type + '\t' + acc_curr + '\t' + currCity + '\t' + priorCity +'\t' + acc_perc.ToString("f1") +"\n"; //"[" + resp_key_list + "]"+ " \t" +  "[" + resp_time_list + "]" 
	Addline();
	
//	
//	if(cntT < G3D_task.numTrials-1){
	cntT++;
//	}
	logTrial  = false;
	cntR = 0;
...
function Addline(){
acc_perc = Mathf.Round(100* (acc_total/(G3D_task.cnt)));
acc_total += acc_curr;		
