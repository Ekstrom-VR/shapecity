...
function Update() {
		...
		if (Input.GetMouseButtonDown(0) && Input.GetKey(KeyCode.LeftControl)) {
			...
			if (Physics.Raycast(ray, hit)) {
				if (hit.collider != null && storeSelected ==false && hit.collider.gameObject.tag == "store"){
...
//						StoreSelect = true;
	...
	if ((Input.GetKey(KeyCode.Return) && storeSelected ==true)||(Input.GetMouseButtonDown(0) && storeSelected ==true)){
						...
						//Change store name to name selected
						currStoreSelect.name = MD_GetStoreName.storeNameCache;
