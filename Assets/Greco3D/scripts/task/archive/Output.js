...
function Update () {
     if(cntT < vars.numT && control.curR < vars.numR ) {
     //Get city type
     trial_order = control.curTrialList[control.curR];
...
if (CityMorph.trial_action == 'task'){
	...
	//Add line	
		...
		Addline();
	 	...
	 	line =  (cntT+1) +  "\t "+ control.curR +"\t " + CityMorph.startTime + "\t" + resp_key + "\t" + resp_time + "\t" + trial_type + '\t' + acc_curr + '\t' + currCity + '\t' + priorCity  +  '\t' + ITI.trialList +  '\t' + ITI.respList +'\t' + PassiveNav_VC.curVidNav +"\n"; 
		Addline();
